...
class VcfFilesExporter(EupathExporter.Exporter):
    """
INPUT
    type specific args:  
...
OUTPUT
  files given cannonical names:
     if file extension is .vcf, no change
     otherwise append .vcf
  ...
  dependency info:
   - reference genome and version (as specified in dedicated ref genome parameter)
    ...
    def initialize(self, stdArgsBundle, typeSpecificArgsList):
        ...
        super().initialize(stdArgsBundle, VcfFilesExporter.TYPE, VcfFilesExporter.VERSION)
        ...
        # print("args to VcfFilesEupathExporter.py", file=sys.stderr)
        ...
        if len(self._refGenomeKey.strip()) == 0 or self._refGenomeKey == VcfFilesExporter.UNSPECIFIED_REF_GENOME_KEY:
        ...
        for i in range(1, len(typeSpecificArgsList), 3):    # start after ref genome arg, increment by tuple size (3)
            ...
            sizeLimit = 1000 * 1024 * 1024 # 1000MB
            # print >> sys.stderr, "file size is " + str(size)
            if size > sizeLimit:
                print("File exceeds 1000MB size limit: " + filename, file=sys.stderr)
                exit(1)

            if refGenomeKey != VcfFilesExporter.UNSPECIFIED_REF_GENOME_KEY and refGenomeKey != self._refGenomeKey:
